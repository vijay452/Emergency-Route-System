...
def _resolve_location(graph: Graph, name: str) -> str:
    """
    Return the canonical location name that matches *name* case-insensitively.

    Falls back to returning *name* unchanged when no match is found (the
    caller is then responsible for raising the appropriate error).
    """
    lower = name.lower()
...
def main() -> None:
    ...
    while True:
        ...
        print("  3. Add a custom location")
        print("  4. Add a custom road")
        print("  5. Exit")
        ...
        if choice == "3":
            loc_name = input("Enter new location name: ").strip()
            if not loc_name:
                print("  Location name cannot be empty.")
                continue
            canonical = _resolve_location(graph, loc_name)
            if canonical in graph.locations:
                print(f"  Location '{canonical}' already exists.")
            else:
                graph.add_location(loc_name)
                print(f"  Location '{loc_name}' added.")
            continue

        if choice == "4":
            _list_locations(graph)
            origin_input = input("Enter origin location: ").strip()
            dest_input = input("Enter destination location: ").strip()
            origin = _resolve_location(graph, origin_input)
            dest = _resolve_location(graph, dest_input)
            try:
                time_str = input("Enter travel time in minutes: ").strip()
                travel_time = float(time_str)
            except ValueError:
                print("  Invalid travel time. Please enter a positive number.")
                continue
            bidir_input = input("Bidirectional road? [y/n] (default y): ").strip().lower()
            bidirectional = bidir_input != "n"
            try:
                graph.add_road(origin, dest, travel_time, bidirectional)
                direction = "↔" if bidirectional else "→"
                print(f"  Road '{origin}' {direction} '{dest}' ({travel_time} min) added.")
        ...
        if choice == "1":
            ...
            start = _resolve_location(graph, start_input)
            end = _resolve_location(graph, end_input)
